...
import { Direction, directionToVector, rotateDirectionCW, oppositeDirection } from '@/utils/Direction';
import type { ITickable } from '@/interfaces/ITickable';
import type { IGridPlaceable } from '@/interfaces/IGridPlaceable';
import type { Grid } from './Grid';
import type { ItemStack } from './ItemStack';

let nextExchangerId = 0;

export type ExchangerMode = 'alternate' | 'filter' | 'ratio';

/**
 * Unified exchanger (Factorio-style).
 ...
 */
export class Exchanger implements ITickable, IGridPlaceable {
  readonly id: string;
  position: Vector2;
  direction: Direction;
  tier: number;
  sleeping: boolean = false;
  mode: ExchangerMode = 'alternate';
  ...
  constructor(position: Vector2, direction: Direction, tier: number = 1) {
    this.id = `exchanger_${nextExchangerId++}`;
  ...
  extractItem(lane: 0 | 1): ItemStack | null {
    if (lane === 0) {
      ...
      this.lane0 = null;
      if (!this.lane1) this.sleep();
      return item;
    } else {
      const item = this.lane1;
      this.lane1 = null;
      if (!this.lane0) this.sleep();
      return item;
    }
  }
